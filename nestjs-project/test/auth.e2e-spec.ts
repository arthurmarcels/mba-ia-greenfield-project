...
describe('Auth (e2e)', () => {
  ...
  async function registerConfirmAndLogin(
    ...
    const res = await request(app.getHttpServer())
      ...
      .send({ email, password });
    return {
      access_token: (res.body as TokenResponseBody).access_token,
      refresh_token: (res.body as TokenResponseBody).refresh_token,
    };
  }
  ...
  describe('POST /auth/login', () => {
    ...
    it('returns 200 with access_token and refresh_token on valid credentials', async () => {
      ...
      expect((res.body as TokenResponseBody).access_token).toBeDefined();
      expect((res.body as TokenResponseBody).refresh_token).toBeDefined();
      expect(typeof (res.body as TokenResponseBody).access_token).toBe(
        'string',
      );
      expect(typeof (res.body as TokenResponseBody).refresh_token).toBe(
        'string',
      );
    });
  ...
  describe('POST /auth/refresh', () => {
    it('returns 200 with new access_token and refresh_token on valid refresh token', async () => {
      ...
      expect((res.body as TokenResponseBody).access_token).toBeDefined();
      expect((res.body as TokenResponseBody).refresh_token).toBeDefined();
      expect((res.body as TokenResponseBody).refresh_token).not.toBe(
        refresh_token,
      );
    });
  ...
  describe('POST /auth/reset-password', () => {
    it('returns 204 with a valid token and updates the password', async () => {
      ...
      const loginRes = await request(app.getHttpServer())
        ...
        .expect(200);
      expect((loginRes.body as TokenResponseBody).access_token).toBeDefined();
    });
